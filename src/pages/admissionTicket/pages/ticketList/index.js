...
import {getTicketList,getSearch} from '../../../../api/ticket'
...
export default class Index extends Component  {
  ...
  _getTicketList() {
    Taro.showLoading({
      ...
      getTicketList(this.state.destinationId,this.state.chuXingDayId,this.state.comprehensiveId,this.state.keyWord,this.state.ps,this.state.pi)
      .then(res=>{
        if(res.Success){
          Taro.hideLoading()
...
// 确定筛选条件
sureSubmit(){
  if (this.state.chuXingDayFlag != -1 || this.state.destinationFlag!=-1) {
...
gotoDetail(ID){
  Taro.navigateTo({
    url: `/pages/admissionTicket/pages/admissionDetail/index?ID=${ID}`
  })
}
  reduction() {//还原初始设置
    const time = 0.5;
...
onShareAppMessage(res) {
  ...
  render() {
    ...
    const {destination,chuXingDay,showSelectFlag,comprehensiveFlag,comprehensiveData,ticketData,clearFlag,showNullFlag} = this.state
    return (
            ...
            <View className='btnBox'>
              <View className="selectBox" onClick={this.showSelect.bind(this)}>
                <View className="itemBox">
                  <Text className={textSelectFlag==true?'selectText selectTextOn':'selectText'}>{this.state.moreSelect}</Text>
              ...
              <View className="selectBox" onClick={this.showComprehensive.bind(this)}>
                <View className="itemBox">
                  <Text className='selectText' className={sortTextFlag==true?'selectText selectTextOn':'selectText'}>{this.state.sortText}</Text>
                  <View className="selectIcon">
                        ...
                        return (
                          ...
                          <View className="proImgBox">
                            ...
                            <View className='dingWeiBox'>
                              ...
                              <Text className='dingWeiCity'>{item.ArriveCityName}</Text>
                            </View>
                          </View>
