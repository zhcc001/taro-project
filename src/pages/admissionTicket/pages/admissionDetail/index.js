...
import {getTicketDetail,likePro,getCanCellike,getlike} from '../../../../api/ticket'
...
import codeIcon from '../../../../common/image/erweima-2.png'
import './index.scss'
export default class Index extends Component  {

   config = {
       navigationBarTitleText: '门票详情页'
  }
...
gotoReserve(){
    ...
    Taro.navigateTo({
        url: `/pages/admissionTicket/pages/reserveTicket/index?ID=${this.state.ID}`
    })
}
backHome() {
  Taro.switchTab({
...
onShareAppMessage(res) {
  ...
  render() {
    const {titleData,maoDianID,isFixed,detailObj,showShare} = this.state
    const scrollStyle = {
      height: '90vh'
    }
    const scrollTop = 0
    ...
    return (
      ...
      <ScrollView
        ...
        {/* <View className="shareCard" onClick={this.showShareCard.bind(this,true)}>
            <View className="codeIcon">
                <Image src={codeIcon} />
            </View>
            <View className="shareText">生成分享图片</View>
        </View> */}
        {showShare&&<View className="saveImg" onClick={this.downloadImg}>
            <View className="saveBtn">保存到手机相册</View>
        </View>}
        {showShare&&<View className="shareContent" id='myCanvas'>
            {/* <Image className='shareProImg' src={proTopImg}/> */}
            <View className="upBox">
                <View className="proText">
                    <View className="proName">{detailObj.Name}</View>
                    <View className="price">
                        <Text className='priceFuHao1'>¥</Text>
                        <Text className='priceNum'>{detailObj.SalePrice}</Text>
                        <Text className="priceFuHao2">/人起</Text>
                    </View>
                    <View className="shareTips"> 微信扫一扫，查看产品详细</View>
                </View>
                <View className="codeBox">
                    {/* <Image className='codeImg' src={proTopImg}/> */}
                </View>
            </View>
        </View>}
        {showShare&&<View className="cabinMask" onClick={this.showShareCard.bind(this,false)}></View>}
    </View>
    );
  }
}